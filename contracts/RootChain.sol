...
contract RootChain {
    ...
    function deposit() public payable {
        ...
        emit Deposit(msg.sender, depositBlock, root, msg.value);
    }
        ...
        public payable onlyOperator
    {
        ...
        bytes32 root = plasmaBlocks[blknum].root;
        bytes32 merkleHash = keccak256(_signedTx);
        //require(Validate.checkSigs(keccak256(_txBytes), root, exitingTx.inputCount, _sigs), "Signatures must match.");
        require(Validate.checkOperatorSig(_txHash, _sigs, operator), "Signatures must match.");
